...
export interface SATDomain {
  ...
  description?: string;
  skills: SATSkill[];
  currentScore: number; // Calculated from skills
  maxScore: number;
}
...
export interface SATSubject {
  ...
  description?: string;
  domains: SATDomain[];
  currentScore: number; // Calculated from domains
  maxScore: number;
}
...
export function calculateOverallScore(): number {
  const totalScore = SAT_STRUCTURE.reduce((sum, subject) => sum + subject.currentScore, 0);
  const maxTotalScore = SAT_STRUCTURE.reduce((sum, subject) => sum + subject.maxScore, 0);
  return Math.round((totalScore / maxTotalScore) * 1600); // SAT is out of 1600
}
