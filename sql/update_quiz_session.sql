-- Function to update quiz session progress (without completing it)
-- This is called after each set of questions to track cumulative progress
...
BEGIN
  ...
  WHERE quiz_sessions.id = p_session_id
    AND quiz_sessions.user_id = auth.uid() -- Ensure user can only update their own sessions
    AND quiz_sessions.is_completed = false -- Only update active sessions
  RETURNING 
    quiz_sessions.id,
...
GRANT EXECUTE ON FUNCTION update_quiz_session(UUID, INT4, INT4, INT4) TO authenticated;